...
"features": ['.combat.on_lh.sulc.sm3.mgh',
...
def patient_data(patient_ids,prep):
    ...
    for pi,patient in enumerate(patient_ids):
        if pi %100==0:
            print(pi)
        left, right = prep.get_data_preprocessed(subject=patient, 
                    features=data_parameters['features'], 
                    lobes = data_parameters['lobes'], lesion_bias=False)
...
def control_data(control_ids,prep):
    ...
    for ci,control in enumerate(control_ids):
        if ci%100==0:
            print(ci)
        left, right = prep.get_data_preprocessed(subject=control, 
                    features=data_parameters['features'], 
                    lobes = data_parameters['lobes'], lesion_bias=False)
...
for t in tf:
    for fold in np.arange(n_folds):
        for exp in exps:
            ...
            yc = np.zeros(n_c)
            yv = np.ones(n_v)
            val_x = np.vstack([save_features[exp][f'p{t}'][fold_step_p*f:fold_step_p*(f+1)],
                            save_features[exp][f'c{t}'][fold_step_c*f:fold_step_c*(f+1)]])
            val_y = np.concatenate([yv[fold_step_p*f:fold_step_p*(f+1)],
                            yc[fold_step_c*f:fold_step_c*(f+1)]])
            train_x = np.vstack([save_features[exp][f'p{t}'][:fold_step_p*f],
                                save_features[exp][f'p{t}'][fold_step_p*(f+1):],
                            save_features[exp][f'c{t}'][:fold_step_c*f],
                            save_features[exp][f'c{t}'][fold_step_c*(f+1):]])
            train_y = np.concatenate([yv[:fold_step_p*f],
                                yv[fold_step_p*(f+1):],
                            yc[:fold_step_c*f],
                            yc[fold_step_c*(f+1):]])
...
results = []
for exp in exps:
    auc = roc_auc_score(    res[exp][f's{t}y'],
                res[exp][f's{t}'])
    print(exp,auc)
