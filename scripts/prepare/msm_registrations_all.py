...
def write_funci_gii(sub_id,hemi,subj):
    ...
    save_numpy_array_as_func_gii(hemi_data, 
...
def write_lesion_mask_gii(sub_id,hemi,subj):
    ...
    save_numpy_array_as_func_gii(lesion_mask.astype(np.int32), 
...
def read_and_write_to_hdf5(hemi,sub_id,subj,hdf5_file_out, features):
    ...
    for fi,feature in enumerate(features):
        ...
        while not done:
            try:
                subj.write_feature_values(feature,
...
def label_to_hdf5(hemi,sub_id,subj,hdf5_file_out):
    ...
    while not done:
        try:
            subj.write_feature_values('.on_lh.lesion.mgh',
