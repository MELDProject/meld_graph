...
for hi,hemi in enumerate(['lh','rh']):
    for si,sub_id in enumerate(subjects):
      ...
      #  save_numpy_array_as_func_gii(os.path.join(p.MELD_DATA_PATH,'msm','msm_regs',f'{hemi}.{sub_id}.func.gii' ),hemi_data)
        np.savetxt(os.path.join(p.MELD_DATA_PATH,'msm','msm_regs',f'{hemi}.{sub_id}.txt' ),hemi_data,)
...
np.savetxt(os.path.join(p.MELD_DATA_PATH,'msm','msm_regs','lh.template.txt'),template,)
np.savetxt(os.path.join(p.MELD_DATA_PATH,'msm','msm_regs','rh.template.txt'),template,)
