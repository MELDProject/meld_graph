...
def save_gifti(filename,surf):
    ...
    parray = nb.gifti.GiftiDataArray(surf['coords'], 
    ...
    tarray = nb.gifti.GiftiDataArray(surf['faces'], 
...
def downsample_mesh(surf):
    ...
    for ti,t0 in enumerate(surf['faces'][triangles_gone]):
        ...
        bool_vec=np.logical_or(copy_faces==t0[2],
                                           np.logical_or(copy_faces==t0[0],
                                                         copy_faces==t0[1])).sum(axis=1)==2
