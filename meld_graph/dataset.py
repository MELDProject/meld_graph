...
class GraphDataset(torch_geometric.data.Dataset):
    def __init__(
        ...
        if self.params["synthetic_data"]["run_synthetic"]:
            ...
            if self.params["synthetic_data"]["n_subs"] < len(self.subject_ids):
                self.subject_ids = np.random.choice(
                    self.subject_ids, self.params["synthetic_data"]["n_subs"]
                )
                ...
                self.subject_samples = np.sort(
                    np.random.choice(
                        np.arange(len(self.subject_ids)),
        ...
        for s_i, subj_id in enumerate(self.subject_ids):
            ...
            (
                ...
                lesion_left,
                lesion_right,
            ) = self.prep.get_data_preprocessed(
                subject=subj_id,
    ...
    def synthetic_lesion(self, features_left=None, features_right=None):
        ...
        for f in [features_left, features_right]:
            ...
            f, l = self.prep.generate_synthetic_data(
                self.icospheres.icospheres[7]["spherical_coords"],
                len(self.params["features"]),
                self.params["synthetic_data"]["bias"],
                self.params["synthetic_data"]["radius"],
                histo_type_seed=subtype,
                proportion_features_abnormal=self.params["synthetic_data"][
                    "proportion_features_abnormal"
                ],
                proportion_hemispheres_abnormal=self.params["synthetic_data"][
                    "proportion_hemispheres_lesional"
                ],
                features=f,
            )
