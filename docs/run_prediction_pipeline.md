...
The MELD graph pipeline can be used to predict FCDs from T1w MRI (and optional FLAIR scans). 

If you wish to use the harmonisation feature of the MELD pipeline, you will need to have first computed the harmonisation parameters for the scanner used to acquire the data. Then you will need to use the harmonisation code when you use the main pipeline command (described below). Please refer to our [guidelines to harmonise a new scanner](https://meld-graph.readthedocs.io/en/latest/harmonisation.html) to learn how to compute the harmonisation parameters. 
...
You only need to change the variable and path that are given in between "<>". For example, change <meld_data> to the path where your meld data folder is stored, and <freesurfer_license> to the path where you have stored the license.txt from Freesurfer. See [installation](https://meld-graph.readthedocs.io/en/latest/install_docker.html) for more details
...
| **Optional variables** |
| ```-harmo_code <harmo_code>```  | provide the harmonisation code if you want to harmonise your data before prediction. This requires harmonisation parameters to have been [computed](https://meld-graph.readthedocs.io/en/latest/harmonisation.html) beforehand. The harmonisation code should start with H, e.g. H1. | 
|```--parallelise``` | use this flag to speed up the segmentation by running Freesurfer/FastSurfer on multiple subjects in parallel. |
|```--fastsurfer``` | use this flag to use FastSurfer instead of Freesurfer. (Requires FastSurfer installed in the native installation). |
|```--skip_feature_extraction``` | use this flag to skips the segmentation and features extraction (processes from script1). Useful if you already have these outputs and you just want to run the preprocessing and the predictions (e.g: after harmonisation) |
...
NOTES: 
- Outputs of the pipeline (mapping the prediction back into the native nifti MRI and MELD reports) are stored in the folder ```output/predictions_reports/<subject_id>```. See [guidelines on how to interepret the results](https://meld-graph.readthedocs.io/en/latest/interpret_results.html) for more details.
...
This script:
 ...
 2. Extracts surface-based features needed for the classifier:
    ...
    * Writes out the features in a hdf5 file
...
This script : 
  1. Smooths features and writes out smoothed features in an hdf5 file
  2. (optional) Harmonises features (using NeuroCombat) and writes harmonised features into the hdf5 file
  2. Normalises the smoothed features (intra-subject & inter-subject (by controls)) and writes out the normalised smoothes features into the hdf5 file
  3. Normalises the raw (harmonised if available) features (intra-subject, asymmetry and then inter-subject (by controls)) and writes out the normalised raw features into the hdf5 file

  Notes: 
  - Features need to have been extracted using script 1. 
  - (optional): this script can also be called to harmonise your data for a new harmonisation code but you will need to pass a file containing the demographics information.
...
This script : 
1. Runs the MELD classifier and predicts lesion on new subject
2. Registers the predictions back into the native nifti MRI. Results are stored in output/predictions_reports/<subjec_id>/predictions.
3. Creates MELD reports with predicted lesion locations on the inflated brain, on the native MRI and computes the associated saliencies. Reports are stored in output/predictions_reports/<subjec_id>/predictions/reports.
